...
const HabitItem: React.FC<HabitItemProps> = ({ task, onToggleStatus, onEdit }) => {
  const isCompleted = task.status === TaskStatus.Completed;

  return (
    <div className={`flex items-center space-x-4 p-3 rounded-xl transition-all duration-300 ${isCompleted ? 'opacity-60 bg-gray-50' : 'bg-white'}`}>
      <div className={`w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 ${categoryStyles[task.category].bg}`}>
      ...
      <button
        ...
        aria-label={`Edit habit: ${task.title}`}
      >
      ...
      <button
        ...
        className={`w-10 h-10 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-all duration-300
          ${isCompleted 
            ? 'bg-green-400 border-green-500' 
            : 'bg-white border-yellow-400 hover:bg-yellow-100'
          }`}
        aria-label={`Mark habit as ${isCompleted ? 'pending' : 'completed'}`}
      >
        {isCompleted && (
